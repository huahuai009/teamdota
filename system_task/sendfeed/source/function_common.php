...
function template($name) {
	...
	if(strexists($name,'/')) {
		...
		$tpl = "template/$_SC[template]/template_$name";
	}
...
function smail($email, $subject, $message='', $from='') {
	...
	if($cid) {
		...
		$mailqueueid = inserttable('mailqueue', $setarr, 1);
		//送入httpsql队列
		include_once(S_ROOT.'./source/class_httpsqs.php');
		$httpsqs = new httpsqs;
		$httpsqs->put($_SC['httpsqs']['server'], $_SC['httpsqs']['port'], $_SC['httpsqs']['charset'], $_SC['httpsqs']['datakey']['sendmail'],urlencode(json_encode(array('method'=>'db','objectid'=>$mailqueueid,'email'=>$email,'subject'=>$subject,'message'=>'','from'=>$from))));
	}
}
//添加数据
...
function sgmmktime($dtime) {
	...
	$mt = @gmmktime($dt[3],$dt[4],$dt[5],$dt[1],$dt[2],$dt[0]) - 3600 * $timeoffset;
	if(!empty($mt)) {
		return $mt;
	} else {
		return $_SC['timestamp'];
	}
}
