...
/*
	...
	$Id: function_sendmail.php 2012-03-31 09:59Z duty $
*/
...
function sendmail($toemail, $subject, $message, $from='') {
	global $_SC, $_SCONFIG, $_SGLOBAL, $group;
	...
	if($mail['mailsend'] == 3) {
		...
		$email_from = $from == '' ? '=?'.$_SC['charset'].'?B?'.base64_encode($_SCONFIG['sitename'])."?= <".$mail['from'].">" : (preg_match('/^(.+?) \<(.+?)\>$/',$from, $mats) ? '=?'.$_SC['charset'].'?B?'.base64_encode($mats[1])."?= <$mats[2]>" : $from);
	}
	...
	if($mail['mailsend'] == 1) {
		...
		fputs($fp, ($mail['auth'] ? 'EHLO' : 'HELO')." teamdota\r\n");
		$lastmessage = fgets($fp, 512);
