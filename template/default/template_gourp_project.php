	...
	<div class="container panel" style="width:1000px; " data-creator-id="<?=$_SGLOBAL['supe_uid']?>">
		...
		<div class="sheet" id="project_data">
			...
			<div class="expanded_content" id="project_data_edit" data-visible-to="admin creator" style="display:none;">
				...
				<div class="position_reference">
						<h1 class="field">
							<textarea cols="40" data-behavior="autoresize submit_on_enter" id="project_name" name="project_name" rows="1"><?=$manageproject['name']?></textarea>
						</h1>
				</div>

				<div class="description field">
						<textarea cols="40" data-behavior="autoresize submit_on_enter" id="project_description" name="project_description" placeholder="添加描述或额外的细节" rows="1"><?=$manageproject['description']?></textarea>
				</div>

				<div class="submit">
						<div id="buttonproject_<?=$project_id?>"><input id="issuance" name="commit" type="button" onclick="validate_create_project('buttonproject_<?=$project_id?>','newproject_<?=$project_id?>','project_edit');" value="保 存" /> 或 <a href="javascript:;" onclick="cancel_project_edit();" class="cancel" data-behavior="cancel" data-role="cancel">取消</a>
						</div>
						<div id="__buttonproject_<?=$project_id?>"></div>
				</div>
				</form>  
			</div>
			<?php } ?>
		...
		<div class="project_toolbar">
			<div class="tools">
				<div class="group">
					...
					<span><strong>|</strong></span>
					<span data-tool-name="members">
						<strong><?=$manageproject['member_num']?>成员</strong>
					</span>
				</div>
			</div>
		</div>
						...
						<article class="todolist" id="todolist_<?=$value['todos_id']?>" data-behavior="expandable">
							<header class="collapsed_content" data-behavior="has_hover_content">
								<div class="nubbin" data-behavior="nubbin hover_content" style="left: -65px; display: none; ">
									...
									<a href="javascript:;" onclick="deletetodos(this,<?=$value['todos_id']?>);" class="image delete" data-confirm="您确定要删除此待办事项清单吗？" rel="nofollow" title="删除">删除</a>
							...
							<ul class="todos  ui-sortable" data-behavior="sortable_container" data-sortable-type="todo">
								...
								<li class="todo show" data-behavior="has_hover_content sortable" data-sortable-type="todo" id="sortable_todo_<?=$valuelist['todoslist_id']?>">
									<div class="nubbin" data-behavior="nubbin hover_content" style="left: -61px; display: none; ">
									  <div class="spacer"></div>
									  <a href="javascript:;" onclick="deletetodoslist(this,<?=$valuelist['todoslist_id']?>);" class="image delete" data-confirm="您确定要删除此待办事项吗？" rel="nofollow" title="删除">删除</a>
		...
		<div class="project_settings" id="div_project_settings">
			...
			<div id="project_settings" style="display:none">
				...
				<input type="hidden" name="formhash" value="<?php echo formhash(); ?>" />
					...
					<div class="submit">
						<div id="button_edit_project_<?=$project_id?>"><input id="issuance_edit_project" name="commit" type="button" onclick="validate_archived_project('button_edit_project_<?=$project_id?>','edit_project_<?=$project_id?>','project_archived');" value="修 改" /><span id="__edit_project_<?=$project_id?>"></span> 或 <a href="javascript:;" onclick="cancel_project_archived();" class="cancel" data-behavior="cancel" data-role="cancel">取消</a>
						</div>
						<div id="__button_edit_project_<?=$project_id?>"></div>
					</div>
				</form>
			</div>
		</div>
		<? } ?>
		<?php if(!$manageproject['discussion_num'] && !$manageproject['file_num'] && !$manageproject['document_num']) {?>
		<article class="blank_slate">
			<div class="blank_slate_body">
				<h1>欢迎来到您的项目！</h1>
				<p>Teamdota可以帮您很好的进行项目协作，开始您的第一次“<a href="cp.php?project_id=<?=$project_id?>&ac=discussion">讨论</a>”，创建一个“<a href="cp.php?project_id=<?=$project_id?>&ac=document&op=create">待办事项清单</a>”，或写一个“<a href="cp.php?project_id=<?=$project_id?>&ac=document&op=create">新的文件</a>“。</p>
			</div>
		</article>
		<?php } ?>
	</div>
