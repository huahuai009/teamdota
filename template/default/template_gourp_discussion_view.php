...
<div class="workspace" id="fileDragArea">
	<div style="width: 980px;" data-container-id="1" class="container stack_container">
		<div class="panel sheet_view inactive <?php if($discussion['status'] == 1) { echo "has_notice is_trashed";}?>" data-behavior=" " data-creator-id="<?=$discussion['uid'] ?>" data-status="<?=get_project_status();?>">
			<header><h1><a href="group.php?do=project&project_id=<?=$project_id?>&discussion_page=<?=$discussion_page?>"><?=$manageproject['name']?> - 交流区</a></h1></header>
			...
			<header class="notice">
				...
				或者 <a href="cp.php?project_id=<?=$project_id?>&ac=<?=$do?>&op=realdelete&discussion_id=<?=$discussion_id ?>&rand=<?=$_SGLOBAL['timestamp'];?>" data-confirm="There is no undo from this action. The content will be lost for good. Are you sure?" data-method="delete" data-remote="true" rel="nofollow">永久删除</a> ，
					  永久删除之后将不可恢复。
				</span>
				<?php } ?>
			</header>
